...
export async function GET(request: NextRequest) {
  try {
    ...
    // If no pagination params, return all jobs
    const jobsQuery = page === 1 && !searchParams.has("limit") ? Job.find(query) : Job.find(query).skip(skip).limit(limit);

    // Execute query
    const jobs = await jobsQuery.sort({ postedDate: -1 });
